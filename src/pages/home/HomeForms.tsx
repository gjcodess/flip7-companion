...
export function JoinGameForm({ name, joinCode, setJoinCode, busy, onJoin }: JoinGameFormProps) {
  return <article className="lobby-card join-card"><span className="eyebrow">JOIN A GAME</span><h2>Have a room code?</h2><p>The host approves every player before the match begins.</p><div className="active-player"><span>Playing as</span><b>{name}</b></div><label>Room code<input value={joinCode} onChange={(e) => setJoinCode(e.target.value.toUpperCase().replace(/[^A-Z0-9]/g, ''))} maxLength={6} placeholder="ABC123" /></label><button className="secondary-wide" disabled={busy !== null || joinCode.length !== 6} onClick={onJoin}>{busy === 'join' ? <LoaderCircle className="spin" /> : <Users />} Request a seat</button></article>
}
