...
export function GameTable({ table, tableCardIds, isVoidedCard, score, flipSevenBonus, busted, frozen, submitting, canEditCards, confirmedAt, isStaying, isOrganized, onOrganize, onOpenPicker, onSelectCard }: GameTableProps) {
  ...
  return <section className="table-area">
    <div className="section-kicker"><Crown size={16} /> MY TABLE <button className="organize-button" onClick={onOrganize} disabled={table.length < 2 || submitting} title={isOrganized ? 'Restore original card order' : 'Organize cards'}><ListOrdered size={14} /> {isOrganized ? 'Original' : 'Organize'}</button></div>
