...
export function CardActionsPanel({ card, submitting, onClose, onEdit, onRemove }: CardActionsPanelProps) {
  return <motion.section className="card-picker card-actions-panel card-focus-panel" initial={{ scale: .86, y: 40, opacity: 0 }} animate={{ scale: 1, y: 0, opacity: 1 }} exit={{ scale: .9, y: 40, opacity: 0 }} transition={{ type: 'spring', stiffness: 330, damping: 25 }} onClick={(event) => event.stopPropagation()}><div className="picker-heading"><div><span>MY CARD</span><h2>{card.label}</h2></div><button className="close-button" aria-label="Close card actions" title="Close" onClick={onClose}><X size={19} /></button></div><motion.div className="card-focus-art" initial={{ scale: .45, y: 100, rotate: -8, opacity: 0 }} animate={{ scale: 1, y: 0, rotate: 0, opacity: 1 }} transition={{ type: 'spring', stiffness: 300, damping: 20, delay: .04 }}><CardArtwork card={card} /></motion.div><p>Edit this card or remove it while your round is still active.</p><div className="card-action-buttons"><button className="secondary-action" disabled={submitting} onClick={onEdit}>Edit card</button><button className="danger-action" disabled={submitting} onClick={onRemove}>Remove card</button></div></motion.section>
}
