-- Keep the round score aligned with the table calculation: additive cards first,
-- then x2. The separate Flip 7 bonus field remains available for future rules UI.
...
begin
  ...
  set number_total = v_numbers,
      ...
      round_score = case when status = 'busted' then 0 else (v_numbers + v_modifiers) * case when v_multiplier then 2 else 1 end end
