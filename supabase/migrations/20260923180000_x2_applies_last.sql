-- Apply additive modifiers before the x2 multiplier, matching the game rules.
...
begin
  ...
  set number_total = v_numbers,
      ...
      round_score = case when status = 'busted' then 0 else (v_numbers + v_modifiers + v_bonus) * case when v_multiplier then 2 else 1 end end
