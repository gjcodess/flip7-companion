-- Flip Three is recorded on the selected target player, including the actor when self-targeted.
...
begin
  ...
  values (case when p_card_code = 'action:flip_three' then v_target_player.id else v_actor_player.id end, p_card_code, v_kind, (select coalesce(max(sequence), 0) + 1 from public.round_cards where round_player_id = case when p_card_code = 'action:flip_three' then v_target_player.id else v_actor_player.id end))
