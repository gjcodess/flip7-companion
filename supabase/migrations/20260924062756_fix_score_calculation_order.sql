-- Apply the Flip 7 scoring steps in order:
-- 1. Add Number cards.
-- 2. Apply the x2 multiplier to that Number-card total.
-- 3. Add +2 through +10 Modifier cards.
-- 4. Add the +15 Flip 7 bonus.
...
begin
  ...
  select
    ...
    count(distinct split_part(card_code, ':', 2)) filter (where card_code like 'number:%'),
